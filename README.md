...
Si uns joueur n'est pas déja présent en base, il vous faut préalablement l'y ajouter en suivant les instructions du "A.1. Création d'une base de joueur."
...
Dans l'application, l'essentiel des options sera accessible sur le menu "20 Gérer les tournois".
...
Le jour du tournoi lorsque les joueurs seront tous présents, vous pourrez "ouvrir" le tournoi sur l'application qui vous proposera la constitution de la première ronde selon l'appariement du tournoi suisse, càd selon leur rang ELO.

Une ronde est soit "encours", "finie" lorsque le chronomètre a été arrétée ou "close" lorsque les résultats ont été saisis.
...
#### __B.3. Mettre à jour les résultatS.__  
...
Sur la base des résultats antérieurs de ce tournoi, l'application calculera un appairement des joueurs pour les rondes suivantes et proposera la répartition des tables de jeu correspondantes.
...
L'application mémorise les données à chaque changement d'option de menu. Vous n'aurez donc pas à lancer manuellement une opération de copie de la base en un fichier d'ordinateur.
...
Par ailleurs, nous vous recommandons de mettre à jour l'application périodiquement pour soit bénéficier d'améliorations, soit limiter les failles de sécurité potentielles.
...
Le logiciel stocke les données dans un fichier texte au format json et un répertoire data. Assurez-vous que le logiciel ait les droits de créer et le répertoire data et le fichier db.json (alternativement dbtest.json).

## Développement

Nous avons tenté de suivre les principes de la programmation orientée objet (classs, encapsulation, héritage) et de respecter le cadriciel MVC en séparant le code en 3 ensembles : Models pour les données, Views pour l'interaction avec l'utilisateur et Controllers pour la logique "Métier".
